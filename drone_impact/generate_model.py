...
Writes:
  building.k   nodes / shells / beams / base node set / facade segment set
  drone.k      nodes / shells / solids / body inner segment set
  blast.k      *LOAD_BLAST_ENHANCED with the battery-centre node baked in
"""
...
def build_building():
    ...
    with open(os.path.join(OUT, "building.k"), "w") as f:
        ...
        w_set_segment(f, SID_SEG_FACADE, seg_facade)
        f.write("$# end building.k\n")

    return m, len(base), len(seg_facade)
...
def build_drone():
    ...
    return m, n_batt, len(seg_drone)


# ============================================================================
def write_blast(nid_batt):
    """LiPo thermal-runaway burst, modelled as a small TNT-equivalent charge
    that travels with the battery (NIDBO)."""
    t = []
    t.append("$# battery burst - generated by generate_model.py")
    t.append("$# 0.030 kg TNT-equivalent ~ fast-release fraction of a 6S/10Ah LiPo")
    t.append("*LOAD_BLAST_ENHANCED")
    t.append("$#     bid         m       xbo       ybo       zbo       tbo      unit     blast")
    t.append("%10d%10s%10s%10s%10s%10s%10d%10d" % (1, "0.030", "0.0", "0.0", "0.0", "7.0", 5, 2))
    t.append("$#     cfm       cfl       cft       cfp     nidbo     death   negphs")
    t.append("%10s%10s%10s%10s%10d%10s%10d" % ("1.0", "0.001", "0.001", "1.0E+9",
                                               nid_batt, "1.0E+20", 0))
    t.append("$")
    t.append("*LOAD_BLAST_SEGMENT_SET")
    t.append("$#     bid      ssid     aleid     sfnrb")
    t.append("%10d%10d" % (1, SID_SEG_DRONE))
    t.append("*LOAD_BLAST_SEGMENT_SET")
    t.append("%10d%10d" % (1, SID_SEG_FACADE))
    t.append("$# end blast.k")
    with open(os.path.join(OUT, "blast.k"), "w") as f:
        f.write("\n".join(t) + "\n")


# ============================================================================
if __name__ == "__main__":
    b, nbase, nsegb = build_building()
    d, nbatt, nsegd = build_drone()
    write_blast(nbatt)
